...
class PrivateIngredientsApiTests(TestCase):
    ...
    def test_create_ingredient_successful(self):
        """Test that create a new ingredient is working properly"""
        payload = {'name': 'banana', 'user': self.user}
        res = self.client.post(INGREDIENTS_URL, payload)
        exists = Ingredients.objects.filter(user=self.user, name=payload['name'])

        self.assertTrue(exists)

    def test_create_ingredient_invalid(self):
        """Test creating a new ingredient with blank payload"""
        payload = {'name': '',}
        res = self.client.post(INGREDIENTS_URL, payload)
